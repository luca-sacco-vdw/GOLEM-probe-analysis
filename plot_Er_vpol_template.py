...
def process_and_plot_dynamic_rho(shots_and_radii, time_limit=9.0, bucket_size=0.25):
    ...
    for i, (start_t, end_t) in enumerate(custom_intervals):
        ...
        eb = plt.errorbar(
            plot_df['x_rho'], plot_df['v_pol'], yerr=plot_df['v_pol_err'],
    ...
    # ==========================================
    # STEP 3: Aesthetics and Export
    # ==========================================
    plt.title(r'$I_p=xx$ kA', fontsize=14)
    plt.xlabel(r'$\rho$ [mm]', fontsize=14)
    plt.ylabel(r"$v_{pol}$ [km/s]", fontsize=14)

    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend(title="t [ms]")
    plt.tight_layout()

    plt.savefig('probe_vpol_template.png', dpi=300)
    plt.show()
